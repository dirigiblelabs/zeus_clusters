...
function handleGetRequest(httpRequest, httpResponse, xss) {
    ...
    if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
        if (id) {
            var entity = clustersDao.get(id);
            if (entity !== null) {
                sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
            } else {
                sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'cluster_id\'=' + id);
            }
        } else if (count !== null) {
            var clustersCount = clustersDao.count();
            sendResponse(httpResponse, httpResponse.OK, 'text/plain', clustersCount);
        } else if (metadata !== null) {
            var clustersMetadata = clustersDao.metadata();
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(clustersMetadata, null, 2));
        } else {
            var clusters = clustersDao.list(limit, offset, sort, desc);
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(clusters, null, 2));
        }
    }
}
...
function handlePutRequest(httpRequest, httpResponse) {
    var entity = getRequestBody(httpRequest);
    var id = getIdParameter(httpRequest, xss);
    id = id !== null ? id : entity.cluster_id;
    if (id !== null) {
        if (clustersDao.get(id) !== null) {
            clustersDao.update(entity);
            sendResponse(httpResponse, httpResponse.NO_CONTENT);
        } else {
            sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'cluster_id\'=' + id);
        }
    } else {
        sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'cluster_id\' parameter is missing!');
    }
...
function getIdParameter(httpRequest, xss) {
    ...
    id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('cluster_id'));
    return id;
}
